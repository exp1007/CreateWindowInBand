    ...
    int nCmdShow)
{
    ...
    // Getting an UIAccess token
    DWORD UI = PrepareForUIAccess();
    ...
    // Creating the window using bands
    ...
    // Checking our current band
    {
        typedef BOOL(WINAPI* GetWindowBand)(_In_ HWND hWnd, _In_ PDWORD pdwBand);
        GetWindowBand pGetWindowBand = reinterpret_cast<GetWindowBand>(GetProcAddress(LoadLibraryA("user32.dll"), "GetWindowBand"));

        DWORD currentBand;
        pGetWindowBand(hWnd, &currentBand);
